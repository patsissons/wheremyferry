<script lang="ts">
  import { groupByRegion, type Data, type Terminal } from '$lib/client';
  ...
  function layoutProps(
    ...
    const regions = Array.from(data.departureRegions.values()).sort(
      ...
      case 'to': {
        if (!selectedFrom) {
          return {
            ...
            regions: Array.from(data.arrivalRegions.values()).sort(
              (a, b) => (b.order ?? 0) - (a.order ?? 0),
            ),
...
<div class="grid h-full grid-rows-[auto,1fr] gap-4">
  <div class="flex flex-col gap-2">
    <h1 class="text-highlight text-2xl font-bold leading-none sm:text-4xl">{title}</h1>
